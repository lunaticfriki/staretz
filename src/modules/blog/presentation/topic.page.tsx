...
import { TopicViewModel } from './viewModels/topic.viewModel';
...
export default function TopicPage({ topic }: TopicPageProps) {
  ...
  const viewModel = useMemo(() => new TopicViewModel(service, topic), [service, topic]);
  ...
  return (
    <div class="container mx-auto px-4 py-8">
      ...
      <h1 class="text-4xl font-bold mb-8 text-zinc-900 dark:text-white">
        Tema: <span class="text-fuchsia-600 dark:text-fuchsia-400">{viewModel.topicName}</span>
      </h1>

      {!viewModel.hasPosts ? (
        <div class="text-center py-10">
          <p class="text-xl text-zinc-500">No s'han trobat articles amb aquest tema.</p>
        </div>
      ) : (
